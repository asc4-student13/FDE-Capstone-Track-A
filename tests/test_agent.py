"""Deterministic tests for procurement recommendation orchestration.

These tests avoid live model calls and validate agent behavior using local tools
and mock data only.
"""
...
def test_approve_req001() -> None:
    ...
    rec: ProcurementRecommendation = evaluate_request(req)
...
def test_deny_budget_overage_req006() -> None:
    """REQ-006: CC-003 has $6,900 remaining; request $11,200 -> deny."""
    req = _make_request(
        request_id="REQ-006",
        requestor="R. Nguyen",
...
def test_deny_policy_catering_req009() -> None:
...
def test_escalate_compliance_flag_req011() -> None:
...
def test_deny_expired_contract_req007() -> None:
...
def test_deny_single_source_violation_req008() -> None:
    """REQ-008: NovaPrint - POL-001 single-source violation -> deny."""
    req = _make_request(
        request_id="REQ-008",
        requestor="L. Torres",
...
def test_escalate_near_director_threshold_req014() -> None:
    """REQ-014: $47,500 hardware - within 5% of $50K threshold -> escalate."""
    req = _make_request(
        request_id="REQ-014",
...
def test_recommendation_always_has_request_id() -> None:
